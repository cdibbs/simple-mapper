...
export function mappable(type: decoratorInput)
{
    return function (target: Object, propertyKey: string | symbol) {
        ...
        let d: { [propKey: string]: decoratorInput } = Reflect.get(target, mapMetadataKey) || {};
        d[propertyKey] = type;
        Reflect.set(target, mapMetadataKey, d);
    };
}

export function getMappableProperties(target: Object): { [propKey : string]: decoratorInput }
{
